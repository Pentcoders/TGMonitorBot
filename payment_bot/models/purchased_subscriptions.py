...
class PurchasedSubscriptions(BaseModel):
    ...
    user_crm:UserCRM = relationship(
        'UserCRM', backref='purchased_subscription', cascade="all", lazy='joined')
    ...
    @staticmethod
    async def get_all_purchased_subscription() -> dict[int, 'PurchasedSubscriptions']:
        async with async_session() as session:
            result = await session.execute(select(PurchasedSubscriptions))
            all_purch: list[PurchasedSubscriptions] = result.scalars().all()
        return {purch.user_crm.tg_user.id_user_tg: purch for purch in all_purch}
